...
func VertexToVertex(nds []*Node) ([]*Node, map[int]*Ends) {
	...
	segUpDwnNode := make(map[int]*Ends, len(nds))
	for i, n := range nds {
		...
		segUpDwnNode[i] = &Ends{USnode: usv, DSnode: tnds[i][0]}
	}
